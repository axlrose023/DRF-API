...
@extend_schema_view(list=extend_schema(
    parameters=[OpenApiParameter('tags',
                                 OpenApiTypes.STR, description='Comma separated list if IDs to filter'),
...
class BaseRecipeAttrViewSet(mixins.UpdateModelMixin,
    ...
    def get_queryset(self):
        assigned_only = bool(int(self.request.query_params.get('asigned_only', 0)))
        queryset = self.queryset
        if assigned_only:
